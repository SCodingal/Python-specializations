...
try:
    dev = AudioUtitlities.GetDefultOutputDevice() if hasattr(AudioUtilities, 'GetDefaultOutputDevice') else AudioUtilities.GetSpeakers()
...
WIN = "Hand Gesture Control"; cv2.nameWindow(WIN, cv2.WINDOW_NORMAL)

while True :
    ok, img = cap.read()
    if not ok: break
    img = cv2.flip(img, 1); h, w, _ = img.shape[:2]
    res = hands.process(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

    if res.multi_hand_landmarks and res.multi_handedness:
        for i, hand in enumerate(res.multi_hand_landamrks):
           ...
           cv2.circle(img, tp, 10, (255,0,0), cv2.FILLED); cv2.circle(img, ip, 10, (0,255,0), cv2.FILLED)
           cv2.line(img, tp, ip, (0,0,255), 3)
           ...
           if label == "Left":
               ...
               cv2.putText(img, f"{int(b)}%", (w-110, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),3)
